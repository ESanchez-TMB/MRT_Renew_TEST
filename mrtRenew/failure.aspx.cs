...
namespace mrtRenew
{
    public partial class failure : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                Session["errMessage"] = "Failure,  Texas.gov failure.";
                Server.Transfer("error.aspx", false);
            }
            //
            if ((app != null) && (app.status.ToUpper().Trim() == "SUCCESS"))
            ...
            if ((!IsCallback) && (Page.PreviousPage == null) && (!IsPostBack))
            {
                ...
                {
                    App_Code.DataAccess.log("failure.aspx: could not chg status of order. orderID=" + ORDERID, "failure.Page_Load");
                    Session["errMessage"] = "Transaction is canceled but status of filure as not updated.";
                    Server.Transfer("error.aspx", false);
                }
            }
            else
